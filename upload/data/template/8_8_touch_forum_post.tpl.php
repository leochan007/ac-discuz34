...
0
|| checktplrefresh('./template/lanmeng_sjyd/touch/forum/post.htm', './template/lanmeng_sjyd/touch/common/seccheck.htm', 1542087683, '8', './data/template/8_8_touch_forum_post.tpl.php', './template/lanmeng_sjyd', 'touch/forum/post')
;?>
<?php if($special != 2 && $special != 4 && !($isfirstpost && $sortid)) { $adveditor = $isfirstpost && $special && ($_GET['action'] == 'newthread' || $_GET['action'] == 'reply' && !empty($_GET['addtrade']) || $_GET['action'] == 'edit' );?><form method="post" id="postform" 
...
<a href="<?php if($_GET['action'] == 'newthread') { ?>forum.php?mod=forumdisplay&fid=<?php echo $_G['fid'];?>&page=<?php echo $_GET['page'];?><?php } else { ?>forum.php?mod=redirect&goto=findpost&ptid=<?php echo $_G['tid'];?>&pid=<?php echo $pid;?><?php } ?>" class="z"><img src="<?php echo $_G['siteurl'];?>template/lanmeng_sjyd/touch/images/icon_back.png" /></a>
<span style="color:#fff;"><?php if($_GET['action'] == 'edit') { ?>编辑<?php } else { ?>发帖<?php } ?></span>
    </div>
</header>
<!-- header end -->
...
<!-- main postbox start -->
<div class="wp">
<div class="post_from lmkj_post">
<ul class="cl">
<li class="lmkj_t_post">
<?php if($_GET['action'] != 'reply') { ?>
<input type="text" tabindex="1" class="px lmkj_h" id="needsubject" size="30" autocomplete="off" value="<?php echo $postinfo['subject'];?>" name="subject" placeholder="标题" fwin="login">
<?php } else { ?>
...
<?php } ?>
<li>
<textarea class="pt lmkj_nei" id="needmessage" tabindex="3" autocomplete="off" id="<?php echo $editorid;?>_textarea" name="<?php echo $editor['textarea'];?>" cols="80" rows="2"  placeholder="内容" fwin="reply"><?php echo $postinfo['message'];?></textarea>
</li>

...
<header class="header">
    <div class="nav">
		...
		<a href="<!--{if $_GET[action] == 'newthread'}-->forum.php?mod=forumdisplay&fid=$_G[fid]&page=$_GET[page]<!--{else}-->forum.php?mod=redirect&goto=findpost&ptid=$_G[tid]&pid=$pid<!--{/if}-->" class="z"><img src="{$_G['siteurl']}template/lanmeng_sjyd/touch/images/icon_back.png" /></a>
		<span style="color:#fff;"><!--{if $_GET[action] == 'edit'}-->{lang edit}<!--{else}-->{lang send_threads}<!--{/if}--></span>
    </div>
</header>
<!-- header end -->
...
<div class="wp">
	<div class="post_from lmkj_post">
		<ul class="cl">
			...
			<!--{if $_GET['action'] != 'reply'}-->
			<input type="text" tabindex="1" class="px lmkj_h" id="needsubject" size="30" autocomplete="off" value="$postinfo[subject]" name="subject" placeholder="{lang thread_subject}" fwin="login">
			<!--{else}-->
				RE: $thread['subject']
			...
			<!--{/if}-->
			<li>
			<textarea class="pt lmkj_nei" id="needmessage" tabindex="3" autocomplete="off" id="{$editorid}_textarea" name="$editor[textarea]" cols="80" rows="2"  placeholder="{lang thread_content}" fwin="reply">$postinfo[message]</textarea>
			</li>

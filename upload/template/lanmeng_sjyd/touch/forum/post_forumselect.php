...
<div class="c lmkj_post"{if empty($_GET['infloat'])} {/if}>
	<p class="cl">
		...
		<!--{if $_G['group']['allowpost'] || !$_G['uid']}-->
			<!--{if $special === null}-->
				<button id="postbtn" class="pn pnc" onclick="hideWindow('nav');showWindow('newthread', 'forum.php?mod=post&action=newthread&fid=' + selectfid)" disabled="disabled"><span style="float:left;width:100px;height:30px;line-height:30px;background:#2E97C2;color:#fff;">{lang send_posts}</span></button>
			<!--{else}-->
				<button id="postbtn" class="pn pnc" onclick="hideWindow('nav');window.location.href='forum.php?mod=post&action=newthread&fid=' + selectfid + '&special=$special'" disabled="disabled"><span style="float:left;width:100px;height:30px;line-height:30px;background:#2E97C2;color:#fff;">$actiontitle</span></button>
			<!--{/if}-->
		<!--{/if}-->
	</p>
	<ul class="pbl cl">
		...
		<li id="block_subforum" style="border:none;"></li>
	</ul>
</div>

...
var page = {
	converthtml : function() {
		...
		if(lastpage) {
			...
			selector += '</select>';
			selector += '<span>绗?+curpage+'椤?/span>';
		}

		$('div.pg').removeClass('pg').addClass('page').html('<a href="'+ prevpagehref +'">涓娄竴椤?/a>'+ selector +'<a href="'+ nextpagehref +'">涓嬩竴椤?/a>');
...
var img = {
	...
	errorhandle : function(obj, is_err_t) {
		...
		if(is_err_t) {
			var parentnode = obj.parent();
			parentnode.find('.loading').remove();
			parentnode.append('<div class="error_text">镣瑰向阅嶆柊锷犺浇</div>');
...
var popup = {
	...
	open : function(pop, type, url) {
		...
		if(typeof pop == 'string') {
			$('#ntcmsg').remove();
			if(type == 'alert') {
				pop = '<div class="tip"><dt>'+ pop +'</dt><dd><input class="button2" type="button" value="纭畾" onclick="popup.close();"></dd></div>'
			} else if(type == 'confirm') {
				pop = '<div class="tip"><dt>'+ pop +'</dt><dd><input class="redirect button2" type="button" value="纭畾" href="'+ url +'"><a href="javascript:;" onclick="popup.close();">鍙栨秷</a></dd></div>'
			}
...
var geo = {
	...
	locationerror : function(error) {
		...
		switch(error.code) {
			case error.TIMEOUT:
				geo.errmsg = "銮峰彇浣岖疆瓒呮椂锛岃阅嶈瘯";
				break;
			case error.POSITION_UNAVAILABLE:
				geo.errmsg = '镞犳碜妫€娴嫔埌镇ㄧ殑褰揿墠浣岖疆';
			    break;
		    case error.PERMISSION_DENIED:
		        geo.errmsg = '璇峰厑璁歌始澶熸宁歌闂偍镄勫綋鍓崭綅缃?;
		        break;
	...
	init : function() {
		...
		.on('touchmove', function(e) {
			...
			if(!status && $(window).scrollTop() <= 0) {
				...
				contentobj.html('<img src="'+ STATICURL + 'image/mobile/images/icon_arrow.gif" style="vertical-align:middle;margin-right:5px;-moz-transform:rotate(180deg);-webkit-transform:rotate(180deg);-o-transform:rotate(180deg);transform:rotate(180deg);"><span id="refreshtxt">涓嬫媺鍙互鍒锋柊</span>');
			...
			if(status == true) {
				...
				if(pullheight >= 0 && pullheight < 150) {
					...
					if(reloadflag) {
						...
						contentobj.find('#refreshtxt').html('涓嬫媺鍙互鍒锋柊');
					}
					reloadflag = false;
					...
					if(!reloadflag) {
						...
						contentobj.find('#refreshtxt').html('鏉惧紑鍙互鍒锋柊');
					}
					reloadflag = true;
				}
			}
		...
		.on('touchend', function(e) {
			if(status == true) {
				if(reloadflag) {
					contentobj.html('<img src="'+ STATICURL + 'image/mobile/images/icon_load.gif" style="vertical-align:middle;margin-right:5px;">姝ｅ湪锷犺浇...');
